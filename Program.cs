...
app.MapPost("/api/products", async (HttpRequest request, MarketplaceDb db, IWebHostEnvironment env) =>
{
    ...
    if (string.IsNullOrWhiteSpace(productRequest.Title))
        return Results.BadRequest(new { message = "Title is required." });

    if (productRequest.Price < 0)
        return Results.BadRequest(new { message = "Price cannot be negative." });

    if (productRequest.Category != "rent" && productRequest.Category != "sale")
        return Results.BadRequest(new { message = "Category must be rent or sale." });

    if (productRequest.Images.Count > 5)
        return Results.BadRequest(new { message = "Maximum 5 images allowed." });
...
{
    return Results.Ok(db.GetContactRequests());
});
